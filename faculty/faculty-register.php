...
if (isset($_POST['submit'])) {
    ...
    if ($password !== $confirmPassword) {
        ...
        $checkIDQuery = "SELECT * FROM tblfaculty WHERE facultyID = ?";
        ...
        if ($resultID->num_rows > 0) {
            echo "<script>alert('The Faculty ID is already taken. ');</script>";
        } else {
            // Check if email is already registered
            $checkEmailQuery = "SELECT * FROM tblfaculty WHERE email = ?";
            ...
            if ($resultEmail->num_rows > 0) {
                ...
                $sql = "INSERT INTO tblfaculty (FacultyID, Name, LastName, FullName, email, MobileNumber, Password) 
                        VALUES ('$facultyID', '$name', '$lastName', '$fullName', '$email', '$mobileNumber', '$hashedPassword')";
...
<script type="text/javascript">
        ...
        // JavaScript function to validate email format for multiple providers (Gmail, Outlook, Yahoo, Yandex)
        function validateEmail() {
            var email = document.getElementById("email").value;
            var emailPattern = /^[a-zA-Z0-9._%+-]+@(gmail\.com|outlook\.com|yahoo\.com|yandex\.ru)$/; // Allowed email domains
    ...
    <div class="form-container">
        <div id="box">
            <form action="" method="post" enctype="multipart/form-data" onsubmit="return validateForm()">
                ...
                <input type="text" name="facultyID" placeholder="Faculty ID" class="box" required>
