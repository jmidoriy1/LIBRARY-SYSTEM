...
$sql = "
    ...
    WHERE 
        (tblissuedbookdetails.ReturnStatus = 0 OR tblissuedbookdetails.ReturnStatus = 1)";
...
if ($query) {
    ...
    if ($results->num_rows > 0) {
        while ($result = $results->fetch_assoc()) {
            // Determine borrowing status based on ReturnStatus and DaysBorrowed/DaysExceeded
            $daysBorrowed = $result['DaysBorrowed'];
            $returnStatus = $result['ReturnStatus'];
            $daysExceeded = $result['DaysExceeded'];

            // Set borrowing status text
            if ($returnStatus == 1) {
            ...
            // Set row background color for overdue books (days exceeded)
            $rowStyle = $daysExceeded > 0 ? 'background-color: red;' : '';

            // Get Name (Faculty name if available, otherwise Student name)
            $name = $result['FacultyID'] ? $result['FacultyName'] : $result['StudentName'];

            // Build the table row, ensuring all values passed to htmlspecialchars are not null
            $table .= '<tr style="' . $rowStyle . '">
                <td><center>' . htmlspecialchars($name ?? '') . '</center></td>
                <td><center>' . htmlspecialchars($result['FacultyID'] ?? $result['StudentID']) . '</center></td> <!-- Display FacultyID if available, otherwise StudentID -->
