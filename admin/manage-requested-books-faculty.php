...
$sortBy = isset($_GET['sortBy']) ? $_GET['sortBy'] : 'f.FacultyID'; // Default sorting by Faculty ID
$order = isset($_GET['order']) ? $_GET['order'] : 'ASC';

// Fetch requested books that match faculty, book numbers, categories, authors, and issue status
$query = "
    SELECT rbd.*, f.FullName, b.BookName, c.CategoryName, a.AuthorName, b.BookNumber, b.issuedcopies 
    FROM tblrequestedbookdetails rbd 
    JOIN tblfaculty f ON rbd.studfacid = f.FacultyID 
            ...
            <div class="row pad-botm">
                <div class="col-md-12">
                    ...
                    <table class='table'>
                        <thead>
                            <tr>
                                <th><a href="?sortBy=f.FacultyID&order=<?php echo ($sortBy == 'f.FacultyID' && $order == 'ASC') ? 'DESC' : 'ASC'; ?>">Faculty ID</a></th>
                                <th><a href="?sortBy=f.FullName&order=<?php echo ($sortBy == 'f.FullName' && $order == 'ASC') ? 'DESC' : 'ASC'; ?>">Faculty Name</a></th>
                        ...
                        if ($result) {
                            while ($row = mysqli_fetch_assoc($result)) {
                                ...
                                if ($row['issuedcopies'] == 1) {
                                    ...
                                    echo "<td><a href='requested-issue-book.php?FacultyID=" . htmlspecialchars($row['studfacid']) . "' class='btn btn-primary'>Issue</a></td>";
                                }
