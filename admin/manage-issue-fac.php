...
$query = "
    SELECT ib.id, ib.IssueDate, ib.ReturnDate, ib.ReturnStatus, s.FacultyID, s.FullName, b.BookNumber, b.BookName,
    ...
    WHERE (s.FullName LIKE '%$searchTerm%' 
        ...
        OR s.FacultyID LIKE '%$searchTerm%'
        OR ib.IssueDate LIKE '%$searchTerm%'
        OR b.BookNumber LIKE '%$searchTerm%')
        AND ib.studorfac = 0
   ORDER BY 
        CASE 
            WHEN BorrowingStatus LIKE 'Exceeded%' THEN 1  -- Exceeded comes first
            WHEN BorrowingStatus LIKE 'Borrowed%' THEN 2  -- Borrowed comes second
            WHEN BorrowingStatus LIKE 'Returned' THEN 3  -- Returned comes last
            ELSE 4  -- In case of any undefined BorrowingStatus, we sort them last
        END $sortOrder, 
...
if (isset($_POST['searchTerm'])) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        $response .= "<tr>
            ...
            <td>
                <a href='edit_issue-fac.php?issue_id=" . htmlspecialchars($row['id']) . "' class='btn btn-primary btn-sm'>Edit</a>
            </td>
        </tr>";
        $cnt++;
    }
...
<head>
    ...
    <title>Admin Dashboard - Manage Issue(Faculty)</title>
                        ...
                        <table class="table table-bordered">
                            <thead>
                                <tr>
                                    ...
                                    <th data-column="s.FacultyID" data-order="ASC">Student ID</th>
                            ...
                            <tbody id="categoryTable">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href="edit_issue-fac.php?issue_id=<?php echo htmlspecialchars($row['id']); ?>" class="btn btn-primary btn-sm">Edit</a>
                                    </td>
                                </tr>
                                <?php endwhile; ?>
                            </tbody>
